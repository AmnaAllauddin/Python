...
class HangmanGame:
    def __init__(self,master):
        ...
        self.word_list=["AMNA","ALLAUDDIN","PYTHON","REACT","JAVASCRIPT","RUBY","CPLUSPLUS"]
    ...
    def setup_alphabet_buttons(self):
                ...
                upper_frame=tk.Frame(self.button_frame)
                upper_frame.pack()
                lower_frame=tk.Frame(self.button_frame)
                lower_frame.pack()
                for letter in upper_row:
                    button=tk.Button(upper_frame,text=letter,command=lambda l=letter:self.guess_letter(l),width=4,height=2,bg=button_bg,font=button_font,fg=button_fg)
                    button.pack(side="left",padx=2,pady=2)
                for letter in lower_row:
                    button=tk.Button(lower_frame,text=letter,command=lambda l=letter:self.guess_letter(l),width=4,height=2,bg=button_bg,font=button_font,fg=button_fg)
                    button.pack(side="left",padx=2,pady=2)
    ...
    def draw_face(self):
                    ...
                    self.hangman_canvas.create_arc(140,85,170,105,start=0,extent=-180, style=tk.ARC,fill="black")
